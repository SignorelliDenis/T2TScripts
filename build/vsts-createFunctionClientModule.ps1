...
$parentModule = 'T2Tscripts'
if (-not $ModuleName) { $ModuleName = 'T2Tscripts.Client' }
...
$publishRoot = Join-Path -Path $WorkingDirectory -ChildPath 'publish\T2Tscripts'
...
$originalManifestData = Import-PowerShellDataFile -Path "$publishRoot\T2Tscripts.psd1"
$prereqHash = @{
