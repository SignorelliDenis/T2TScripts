...
#>
	...
	process
	{
		try { $uri = '{0}{1}' -f (Get-PSFConfigValue -FullName 'T2Tscripts.Client.Uri' -NotNull), $FunctionName }
		catch { $PSCmdlet.ThrowTerminatingError($_) }
		$header = @{ }
		
		#region Authentication
		$unprotectedToken = Get-PSFConfigValue -FullName 'T2Tscripts.Client.UnprotectedToken'
		$protectedToken = Get-PSFConfigValue -FullName 'T2Tscripts.Client.ProtectedToken'
